...
class MeshDataset(Dataset):
    ...
    '''
    def load_cloud(self, scroll_name):
        scroll = Volume(scroll_name)
        print(f"zarr shape: {scroll.shape}")
        return scroll'''

    def load_tifstack(self, path):
        # Create a glob pattern to match all .tif files
        pattern = os.path.join(path, "*.tif")
        # Use imread with the pattern
    ...
    def load_grid(self, grid_index, uint8=False):
        ...
        elif self.scroll_format in ["zarr", "cloud"]:
    ...
    def load_grid_dask(self, grid_index, uint8=False):
        ...
        # Compute the shape of the data to load
        cell_shape = (grid_end[0] - grid_start[0],
                    grid_end[1] - grid_start[1],
                    grid_end[2] - grid_start[2])

        # Load the data slice from the Dask array and compute it
        zarr_grid = self.zarr[grid_start[0]:grid_end[0],grid_start[1]:grid_end[1],grid_start[2]:grid_end[2]].compute()

        # Create a NumPy array with zeros and assign the loaded data
        grid_cell = np.zeros((self.grid_size, self.grid_size, self.grid_size), dtype=np.uint16)
...
def main():
    ...
    parser.add_argument('scroll', type=str, help="Path to the grid cells, tifstack, zarr or vesuvius file")
